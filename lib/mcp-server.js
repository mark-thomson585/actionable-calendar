...
function buildServer() {
  ...
  server.tool(
    "add_item",
    "Add a task or goal to Mark's actionable-calendar. Tasks should have a date (YYYY-MM-DD); goals omit date entirely. For a single time-of-day, set start_time only. For a time range, set both start_time and end_time. Use repeat_rule 'weekday' for a Monday-Friday routine. Set no_rollover for items that should never nag forward when left unchecked — they just jump straight to their next scheduled occurrence instead.",
    {
      ...
      repeat_rule: z.enum(["daily", "weekday", "weekly", "monthly"]).optional(),
